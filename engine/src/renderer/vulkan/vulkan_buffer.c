...
)
{
    ...
    {
        KERROR("vulkan_buffer_resize() requires that new_size be larger than the old. Not doing this could lead to data loss!");
        return false;
    }
    ...
    // Make sure anything potentially using them is finished.
...
b8 vulkan_buffer_allocate(vulkan_buffer* buffer, u64 size, u64* out_offset)
{
    ...
    {
        KERROR("vulkan_buffer_allocate() requires valid buffer, a non-zero size and valid pointer to hold offset!");
        return false;
    }
...
b8 vulkan_buffer_free(vulkan_buffer* buffer, u64 size, u64 offset)
{
    if (!buffer || !size)
    {
        KERROR("vulkan_buffer_free() requires a valid buffer and a non-zero size!");
        return false;
    }

    if (!buffer->has_freelist)
    {
        KWARN("vulkan_buffer_free() called on a buffer not using freelists. Nothing was done.");
        return true;
    }
