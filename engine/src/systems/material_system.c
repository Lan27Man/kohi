...
void material_system_shutdown(void* state)
{
    if (state_ptr)
    {
        ...
        for (u32 i = 0; i < count; ++i)
        {
            ...
            {
                destroy_material(&state_ptr->registered_materials[i]);
            }
        }
...
void material_system_release(const char* name)
{
    ...
    if (state_ptr && hashtable_get(&state_ptr->registered_material_table, name, &ref))
    {
        ...
        {
            KTRACE("Released material '%s', now has a reference_count of '%i' (auto_release = %s).", name, ref.reference_count, ref.auto_release);
        }
...
material* material_system_get_default_material()
{
    ...
    KFATAL("material_system_get_default_material() called before system is initialized!");
    return 0;
}
...
b8 load_material(material_config config, material* m)
{
    ...
    {
        // NOTE: Only set for clarity, as the call to kzero_memory() above does this already.
...
b8 load_configuration_file(const char* path, material_config* out_config)
{
    ...
    while (filesystem_read_line(&f, 511, &p, &line_length))
    {
        ...
        if (equal_index == -1)
        {
            KWARN("Potential formatting issue found in file '%s': '=' token not found. Skipping line %u.", path, line_number);
