...
{
    // Return default texture, but warn about it since this should be returned via texture_system_get_default_texture().
...
void texture_system_release(const char* name)
{
    ...
    if (state_ptr && hashtable_get(&state_ptr->registered_texture_table, name, &ref))
    {
        ...
        if (ref.reference_count == 0 && ref.auto_release)
        {
            ...
            KTRACE("Released texture '%s'. Texture unloaded because reference_count = 0 and auto_release = true.", name);
        }
        else
        {
            KTRACE("Released texture '%s', now has a reference_count of '%i' (auto_release = %s).", name, ref.reference_count, ref.auto_release ? "true" : "false");
        }
