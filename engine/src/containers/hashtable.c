...
b8 hashtable_set_ptr(hashtable* table, const char* name, void** value)
{
    if (!table || !name)
    {
        KERROR("hashtable_set_ptr() requires table and name to exist!");
        return false;
    }
...
b8 hashtable_get_ptr(hashtable* table, const char* name, void** out_value)
{
    if (!table || !name || !out_value)
    {
        KERROR("hashtable_get_ptr() requires table, name and out_value to exist!");
        return false;
    }
...
b8 hashtable_fill(hashtable* table, void* value)
{
    if (!table || !value)
    {
        KERROR("hashtable_fill() requires table and value to exist!");
        return false;
    }
