...
b8 freelist_allocate_block(freelist* list, u64 size, u64* out_offset)
{
    ...
    KWARN("freelist_allocate_block() - No block with enough free space found (requested: %lluB, available: %lluB).", size, free_space);
    return false;
}
...
b8 freelist_free_block(freelist* list, u64 size, u64 offset)
{
    ...
    KWARN("Unable to find a block to be freed. Corruption possible?");
    return false;
}
...
void freelist_clear(freelist* list)
{
    ...
    // Reset the head to occupy the whole thing.
    state->head->offset = 0;
